...
export const saveCwd = (cwd: string) => {
  store.set("cwd", cwd);
};
...
export const startupData = () => {
  ...
  getWindow().webContents.send(
    "data",
    JSON.stringify({ type: "store-data", payload: store.store })
  );
};
...
const loadIpcEvents = () => {
  ...
  ipcMain.on(IPC_EVENTS.SAVE_CWD, (event, args) => {
    saveCwd(args.cwd);
  });
  ipcMain.on(IPC_EVENTS.STARTUP_DATA, (event, args) => {
    startupData();
  });
};
